...
1. sequence is at least `r minlength`bp in length and no greater than `r maxlength`bp in length; 
2. max hamming distance of barcode = `r maxhammingdisttag`; 
3. maximum number of matching error to forward primer sequences = `r maxforwardprimererrors`; 
4. had < `r maxNs` ambiguous bases (Ns); - After Lucy clipping however, no Ns occur
5. had < `r maxhomopol`bp homopolymer run in sequence; 
6. alignment to the silva bacterial database is within 75bp of the expected alignment start position (507bp); and 
7. read alignment started within the first 5bp and extended through read to within the final 5bp. 
...
Reads Greater than Min Length (`r minlength`) :: Reads meeting that criteria: `r table(factor(ReadData$LucyLength > minlength,levels=c(FALSE,TRUE)))[2]`

Reads Less than Max Length (`r maxlength`) :: Reads meeting that criteria: `r table(factor(ReadData$LucyLength < maxlength,levels=c(FALSE,TRUE)))[2]`

Maximum Number of Abiguous Characters (Ns) <= `r maxNs` :: Reads meeting that criteria: `r table(factor(ReadData$LucyNs <= maxNs,levels=c(FALSE,TRUE)))[2]`

Maximum Length of a Homopolymer Run <= `r maxhomopol` :: Reads meeting that criteria: `r table(factor(ReadData$LucymHomoPrun <= maxhomopol,levels=c(FALSE,TRUE)))[2]`
